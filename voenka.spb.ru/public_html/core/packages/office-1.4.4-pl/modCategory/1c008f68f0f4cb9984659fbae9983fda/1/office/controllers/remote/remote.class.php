...
class officeRemoteAuthController extends officeAuthController
{
    ...
    public function defaultAction()
    {
        if ($this->modx->user->isAuthenticated($this->modx->context->key)) {
            ...
            $output = $this->office->getChunk($this->config['tplLogout'], $pls);
        } else {
            if (empty($this->config['remote'])) {
                ...
                $output = $this->office->getChunk($this->config['tplLogin'], array(
    ...
    public function getCypher()
    {
        if (!empty($this->config['key'])) {
            ...
            $Cypher = false;
            $this->modx->log(modX::LOG_LEVEL_ERROR,
                '[Office] Could not protect data: you must specify the same nonempty keys at server and client. For example "&key=`8Hy76Jkw`".');
        }
